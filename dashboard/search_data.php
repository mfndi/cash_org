...
// $see = new Config_uangkas();
// $daftar_uangkas = $see->view_uangkas();
...
if(isset($_GET['cari']) && !empty($_GET['cari'])){
    $data = $_GET['cari'];
    $db = database::connect();
        $query = $db->prepare("SELECT * FROM cash_masuk WHERE bayar_anggota LIKE :nama or nominal_cash LIKE :nominal or ket_pembayaran LIKE :ket or tgl_masuk LIKE :tgl");
        $query->bindValue(":nama", "%" . $data . "%", PDO::PARAM_STR);
        $query->bindValue(":nominal", "%" . $data . "%", PDO::PARAM_STR);
        $query->bindValue(":ket", "%" . $data . "%", PDO::PARAM_STR);
        $query->bindValue(":tgl", "%" . $data . "%", PDO::PARAM_STR );
        $query->execute();
            if($query->rowCount() === 1){
                $data_kas = $query->fetch(PDO::FETCH_ASSOC);
                $nama_anggota = $data_kas['bayar_anggota'];
                $jumlah = $data_kas['nominal_cash'];
                $tanggal = $data_kas['tgl_masuk'];
                $catatan = $data_kas['ket_pembayaran'];
                $id = $data_kas['id'];
                $code = $data_kas['code_uniq'];
           } else{
            echo "<script>alert('Tidak ada data!');
                              document.location.href = 'daftar_keuangan.php';</script>";
            }
                    ...
                    <center>
                        ...
                        <div class="card-body">
                             ...
                             <div class="col-md-8 mt-5 ">
                               <form class="form-inline my-2 my-lg-0" action="search_data.php" method="get" name='cari'>
                                ...
                                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>&nbsp;<a href="edit_kas.php?num=<?php echo $code; ?>"><button type="button" class="btn btn-success">Edit</button></a>
                                                &nbsp; <a href="hapus_data.php?id_kas=<?php echo $code; ?>"><button type="button" class="btn btn-danger" onclick="return confirm('Apakah Anda Yakin?')">Hapus</button></a>
                                            </td>
                                        </tr>
                                    </tbody>
                                    <?php $no++; ?>
