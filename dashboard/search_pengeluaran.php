...
$catatan_update = new Config_uangkas();
...
if(isset($_GET['cari']) && !empty($_GET['cari'])){
    $cari = $_GET['cari'];
    $db = database::connect();
        $query = $db->prepare("SELECT * FROM cash_catatan WHERE catatan LIKE :catatan OR nominal_uang LIKE :nominal OR tanggal LIKE :tgl");
        $query->bindValue(":catatan", "%" . $cari . "%", PDO::PARAM_STR);
        $query->bindValue(":nominal", "%" . $cari . "%", PDO::PARAM_STR);
        $query->bindValue(":tgl", "%" . $cari . "%", PDO::PARAM_STR);
        $query->execute();
            if($query->rowCount() === 1){
                $hasil = $query->fetch(PDO::FETCH_ASSOC);
                    $catatan = $hasil['catatan'];
                    $nominal = $hasil['nominal_uang'];
                    $tanggal = $hasil['tanggal'];
                    $id = $hasil['id'];
                    $code = $hasil['code_uniq'];
            }else{
                                ...
                                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>&nbsp;<a href="edit_pengeluaran.php?catatan=<?php echo $code; ?>"><button type="button" class="btn btn-success">Edit</button></a>
                                                &nbsp; <a href="hapus_data.php?catatan=<?php echo $code; ?>"><button type="button" class="btn btn-danger" onclick="return confirm('Apakah Anda Yakin?')">Hapus</button></a>
                                            </td>
                                        </tr>
                                    </tbody>
                                <?php $no++; ?>
