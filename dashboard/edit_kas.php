...
$data_anggota = new Show_anggota();
$kas_masuk = new Config_uangkas();
$all_anggota = $data_anggota->view_anggota();
...
if(isset($_GET['num']) && !empty($_GET['num'])){
    $id_data = $_GET['num'];
    $db = database::connect();
    $query = $db->prepare("SELECT * FROM cash_masuk WHERE code_uniq = :id");
    $query->bindParam(":id", $id_data);
    $query->execute();
        if($query->rowCount() === 1){
            $isi_data = $query->fetch(PDO::FETCH_ASSOC);
                $nama_anggota = $isi_data['bayar_anggota'];
                $nominal_cash = $isi_data['nominal_cash'];
                $tgl_kas = $isi_data['tgl_masuk'];
                $catatan = $isi_data['ket_pembayaran'];
                $id = $isi_data['id'];
...
if(isset($_POST['submit'])){
    if($kas_masuk->update_uangkas($_POST) > 0){
...
<!--                                     <div class="text-center">
                                    ...
                                    <form class="user" method="post" action="">
                                        ...
                                        <div class="form-group">
                                            <select class="form-control" name="bayar_anggota">
                                            <option selected value="<?php echo $nama_anggota; ?>"><?php echo $nama_anggota; ?></option>
                                        ...
                                        <div class="form-group">
                                            <input type="number" class="form-control" name="nominal_cash" value="<?php echo $nominal_cash; ?>">
                                        </div>
                                        <div class="form-group">
                                            <input type="date" class="form-control" name="tgl_kas" value="<?php echo $tgl_kas; ?>">
                                        </div>
                                        <div class="mb-3">
